...
export const DashboardLayout = ({children}: DashboardLayoutProps) => {
    ...
    return (
        <div className={'flex'}>
            <div
                className={`h-screen border  ${openSidebar ? 'w-[300px]' : 'w-[0px]'} transition-[width] duration-300 ease-in-out flex flex-col items-center justify-center`}>
                <div
                    className={`flex flex-col h-[calc(90%+60px)]  px-5 w-full ${!openSidebar && 'hidden'}  overflow-hidden`}>
                    <div>
                        <DefaultButton size={'large'} className={'w-full'}><RoundedPlus color={'white'}/>New
                    ...
                    <div className={'mt-auto'}>
                        <h1 className={'font-light text-[24px] cursor-pointer leading-[40px] flex items-center gap-2'}>
                            <AlarmIcon/>Notification</h1>
                        <h1 className={'font-light text-[24px] cursor-pointer  flex items-center gap-2'}><SupportIcon/>Support
                        </h1>

                        <DefaultButton size={'large'} buttonType={'outline'} className={'w-full'}><RoundedPlus
